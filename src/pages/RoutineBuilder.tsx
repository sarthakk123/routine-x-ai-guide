...
export default function RoutineBuilder() {
  ...
  // Use the toast from sonner instead of the shadcn/ui one to avoid circular reference
  ...
  const loadSavedRoutines = async () => {
    ...
    try {
      // Check if a "routines" table exists in Supabase first
      // For now, we'll just set an empty array to prevent TypeScript errors
      console.log("Would load routines for user:", user.id);
      setSavedRoutines([]);
      
      /* In a real application, uncomment this after creating the routines table
      ...
      if (data) {
        setSavedRoutines(data as Routine[]);
      }
      */
    } catch (error) {
  ...
  const generateRoutine = async () => {
    ...
    try {
      ...
      toast.error('Error generating routine', {
        description: 'Please try again later.'
      });
    } finally {
      setIsGenerating(false);
